...
async function notify(type, data) {
  const messages = {
    ...
    hot_lead: {
      text: `*HOT LEAD ACTIONED*\n*Name:* ${data.name}\n*Title:* ${data.title} at ${data.company}\n${data.isHotSpike ? `🔥 *HOT SPIKE: ${data.domainVisits} recent visits from this domain!*\n` : ''}*ICP Score:* ${data.icpScore}/100 (${data.leadTier})\n*Pages visited:* ${data.pagesVisited}\n*LinkedIn:* ${data.linkedinUrl}\n*AI Icebreaker:* _"${data.icebreaker}"_\n*Actions Taken:* HeyReach connected, Instantly emailed, HubSpot Company & Contact created.`
    }
  };
